          ...
          <div className="px-5 py-4 space-y-4">
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between">
                ...
                {(() => {
                  const activeCal = calendars?.find(c => c.calendar_id === selectedCalendarId)
                  if (activeCal && !activeCal.is_default) {
                    return (
                      <button
                        type="button"
                        onClick={() => setDefault.mutate({ calendarId: selectedCalendarId })}
                        className="text-[11px] font-semibold text-terracotta-500 hover:text-terracotta-600 transition-colors"
                      >
                        Make default
                      </button>
                    )
                  } else if (activeCal?.is_default) {
                    return (
                      <span className="text-[10px] text-brown-700/40 bg-sand-100 rounded px-1.5 py-0.5 font-medium">
                        default
                      </span>
                    )
                  }
              ...
              <div className="relative">
                <select
                  ...
                  className={`w-full rounded-xl border border-sand-200 bg-cream-50 px-3 py-2.5 text-sm text-brown-800 focus:border-terracotta-500 focus:outline-none appearance-none pr-10 ${isEdit ? 'opacity-70 cursor-not-allowed' : ''}`}
                >
                  <option value="" disabled className="text-brown-700/30">
                    Select a calendar
                  </option>
                  {Array.from(calsByMember.entries()).map(([ownerName, cals]) => (
                    <optgroup key={ownerName} label={ownerName} className="font-semibold text-brown-800">
                      {cals!.map(cal => (
                        <option key={cal.id} value={cal.calendar_id} className="text-brown-700 font-normal">
                ...
                <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-brown-700/40">
